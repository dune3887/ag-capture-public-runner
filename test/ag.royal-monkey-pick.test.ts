...
const session=()=>new RoxorCometDSession({gameId:'test',name:'test',backendArtifactId:'rgp-game-gold-stacks-88-royal-monkey'});
test('Royal Monkey 免费选择使用1..3协议ID，忽略旧奖池字段',()=>{
 ...
 assert.equal(p.kind,'choice');assert.deepEqual(p.options.map(o=>o.requestPickIndex),[1,2,3]);assert.equal(p.remapReveal,undefined);
 assert.throws(()=>session().getPickProtocol('PICK',{NextActionInfo:{id:'UNKNOWN'}}),/unknown Royal Monkey Pick branch/);
});
for(const choice of [1,2,3])test('Royal Monkey 奖池后免费选择 '+choice+' 不继承揭示索引',async()=>{
 ...
 assert.deepEqual(steps[0].selectableIndexes,[]);assert.deepEqual(steps[1].selectableIndexes,[1,2,3]);
});
